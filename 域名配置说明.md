# 域名配置说明

## 🌐 使用的域名

本项目使用 `edm.sendwalk.com` 子域名部署：

| 服务 | 域名 | 说明 |
|------|------|------|
| 前端应用 | `edm.sendwalk.com` | 用户访问的主界面 |
| 后端 API | `api.edm.sendwalk.com` | API 接口服务 |

## 📋 DNS 配置

需要在域名 DNS 管理中添加以下 A 记录：

```
# 假设服务器 IP 为: 1.2.3.4

A     edm           1.2.3.4
A     api.edm       1.2.3.4
```

或者使用 CNAME（如果主域名已有 A 记录）：

```
CNAME edm           sendwalk.com
CNAME api.edm       sendwalk.com
```

## ✅ 已更新的配置文件

### 1. **Nginx 前端配置** (`nginx/frontend.conf`)

```nginx
server {
    listen 80;
    server_name edm.sendwalk.com;
    root /data/www/sendwalk/frontend/dist;
    ...
}
```

### 2. **Nginx API 配置** (`nginx/api.conf`)

```nginx
server {
    listen 80;
    server_name api.edm.sendwalk.com;
    root /data/www/sendwalk/backend/public;
    ...
}
```

### 3. **后端环境配置** (`config.env.production.example`)

```bash
APP_URL=https://edm.sendwalk.com
FRONTEND_URL=https://edm.sendwalk.com
SANCTUM_STATEFUL_DOMAINS=edm.sendwalk.com
SESSION_DOMAIN=.edm.sendwalk.com
```

### 4. **前端环境配置**

创建 `frontend/.env.production`：

```bash
VITE_API_URL=https://api.edm.sendwalk.com
VITE_APP_NAME=SendWalk
```

## 🚀 部署步骤

### 1. 配置 DNS

在域名服务商（如阿里云、腾讯云、Cloudflare）添加 DNS 记录：

```
类型   主机记录      记录值
A      edm          服务器IP
A      api.edm      服务器IP
```

等待 DNS 生效（通常 5-10 分钟，最长可能需要 24 小时）。

### 2. 验证 DNS

```bash
# 检查前端域名
ping edm.sendwalk.com

# 检查 API 域名
ping api.edm.sendwalk.com

# 或使用 nslookup
nslookup edm.sendwalk.com
nslookup api.edm.sendwalk.com
```

### 3. 部署代码

按照正常流程部署代码到 `/data/www/sendwalk`。

### 4. 配置后端环境变量

```bash
cd /data/www/sendwalk/backend
cp /data/www/sendwalk/config.env.production.example .env

# 编辑 .env 文件
nano .env
```

确保以下配置正确：

```bash
APP_URL=https://edm.sendwalk.com
FRONTEND_URL=https://edm.sendwalk.com
SANCTUM_STATEFUL_DOMAINS=edm.sendwalk.com
SESSION_DOMAIN=.edm.sendwalk.com
```

### 5. 配置前端环境变量

```bash
cd /data/www/sendwalk/frontend

# 创建生产环境配置
cat > .env.production << 'EOF'
VITE_API_URL=https://api.edm.sendwalk.com
VITE_APP_NAME=SendWalk
EOF

# 或者编辑 .env 文件
nano .env
```

内容：

```bash
VITE_API_URL=https://api.edm.sendwalk.com
VITE_APP_NAME=SendWalk
```

### 6. 重新构建前端

```bash
cd /data/www/sendwalk/frontend
npm run build
```

### 7. 配置 Nginx

```bash
# 复制配置文件到 conf.d 目录
sudo cp /data/www/sendwalk/nginx/api.conf /etc/nginx/conf.d/sendwalk-api.conf
sudo cp /data/www/sendwalk/nginx/frontend.conf /etc/nginx/conf.d/sendwalk-frontend.conf

# 测试配置
sudo nginx -t

# 重启 Nginx
sudo systemctl restart nginx
```

### 8. 配置 SSL 证书

#### ✅ 使用已有的 Cloudflare 证书（推荐）

本项目已配置使用 Cloudflare 生成的证书：

```bash
# 证书文件位置
/data/www/ca/sendwalk.pem   # 证书文件
/data/www/ca/sendwalk.key   # 私钥文件

# 确认证书文件存在
ls -lh /data/www/ca/sendwalk.pem
ls -lh /data/www/ca/sendwalk.key

# 设置正确的权限
sudo chown root:root /data/www/ca/sendwalk.*
sudo chmod 644 /data/www/ca/sendwalk.pem
sudo chmod 600 /data/www/ca/sendwalk.key

# Nginx 配置已包含 SSL 设置，直接重启即可
sudo nginx -t
sudo systemctl restart nginx
```

> 💡 **说明**: Nginx 配置文件已经包含完整的 SSL 配置，包括：
> - HTTPS 监听（443 端口）
> - SSL 证书路径
> - 安全的 TLS 协议和加密套件
> - HTTP 到 HTTPS 自动重定向
> - 安全头（HSTS、X-Frame-Options 等）

详细的 SSL 配置说明请查看：[SSL证书配置说明.md](./SSL证书配置说明.md)

#### 或者：使用 Let's Encrypt（备选方案）

如果要使用 Let's Encrypt 免费证书：

```bash
# 安装 Certbot
sudo apt install certbot python3-certbot-nginx

# 申请证书（会自动修改 Nginx 配置）
sudo certbot --nginx -d edm.sendwalk.com
sudo certbot --nginx -d api.edm.sendwalk.com
```

> ⚠️ **注意**: 使用 Let's Encrypt 会覆盖 Nginx 配置中的 SSL 设置。

### 9. 验证部署

```bash
# 测试前端（HTTP）
curl -I http://edm.sendwalk.com

# 测试前端（HTTPS）
curl -I https://edm.sendwalk.com

# 测试 API（HTTP）
curl http://api.edm.sendwalk.com/api/health

# 测试 API（HTTPS）
curl https://api.edm.sendwalk.com/api/health
```

### 10. 浏览器测试

在浏览器中访问：

- 前端：`https://edm.sendwalk.com`
- API 健康检查：`https://api.edm.sendwalk.com/api/health`

## 🔧 SSL 证书配置详解

### 自动配置（推荐）

```bash
# Certbot 会自动配置 SSL 和 HTTP 到 HTTPS 的重定向
sudo certbot --nginx -d edm.sendwalk.com
sudo certbot --nginx -d api.edm.sendwalk.com
```

### 手动配置

如果需要手动配置，Nginx SSL 配置示例：

```nginx
server {
    listen 443 ssl http2;
    server_name edm.sendwalk.com;
    
    ssl_certificate /etc/letsencrypt/live/edm.sendwalk.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/edm.sendwalk.com/privkey.pem;
    
    ssl_protocols TLSv1.2 TLSv1.3;
    ssl_ciphers HIGH:!aNULL:!MD5;
    ssl_prefer_server_ciphers on;
    
    # 其他配置...
}

# HTTP 重定向到 HTTPS
server {
    listen 80;
    server_name edm.sendwalk.com;
    return 301 https://$server_name$request_uri;
}
```

### 证书自动续期

Let's Encrypt 证书有效期 90 天，Certbot 会自动创建续期任务：

```bash
# 检查自动续期配置
sudo systemctl status certbot.timer

# 测试续期（不会真正续期）
sudo certbot renew --dry-run

# 手动续期
sudo certbot renew
```

## 🔍 故障排查

### 问题 1：域名无法访问

**检查**：
```bash
# 检查 DNS 是否生效
nslookup edm.sendwalk.com
nslookup api.edm.sendwalk.com

# 检查 Nginx 配置
sudo nginx -t

# 检查 Nginx 是否运行
sudo systemctl status nginx
```

**解决方案**：
- 等待 DNS 生效
- 检查防火墙是否开放 80/443 端口
- 检查 Nginx 配置是否正确

### 问题 2：前端可以访问，但无法调用 API

**检查**：
```bash
# 检查 API 是否可访问
curl https://api.edm.sendwalk.com/api/health

# 检查前端配置
cat /data/www/sendwalk/frontend/dist/assets/*.js | grep -o 'api\.edm\.sendwalk\.com'
```

**解决方案**：
- 检查前端 `.env` 中的 `VITE_API_URL`
- 重新构建前端：`npm run build`
- 检查后端 CORS 配置

### 问题 3：CORS 错误

**检查后端配置**：
```bash
cd /data/www/sendwalk/backend
grep -E "FRONTEND_URL|SANCTUM|SESSION" .env
```

应该包含：
```bash
FRONTEND_URL=https://edm.sendwalk.com
SANCTUM_STATEFUL_DOMAINS=edm.sendwalk.com
SESSION_DOMAIN=.edm.sendwalk.com
```

**解决方案**：
```bash
# 更新配置后清除缓存
php artisan config:clear
php artisan config:cache

# 重启 PHP-FPM
sudo systemctl restart php8.3-fpm
```

### 问题 4：SSL 证书错误

**检查证书**：
```bash
# 检查证书有效期
sudo certbot certificates

# 查看证书详情
openssl x509 -in /etc/letsencrypt/live/edm.sendwalk.com/fullchain.pem -text -noout
```

**解决方案**：
```bash
# 重新申请证书
sudo certbot certonly --nginx -d edm.sendwalk.com
sudo certbot certonly --nginx -d api.edm.sendwalk.com

# 重启 Nginx
sudo systemctl restart nginx
```

## 📊 访问地址汇总

| 服务 | HTTP | HTTPS | 说明 |
|------|------|-------|------|
| 前端 | http://edm.sendwalk.com | https://edm.sendwalk.com | 主应用 |
| API | http://api.edm.sendwalk.com | https://api.edm.sendwalk.com | API 接口 |
| API 健康检查 | - | https://api.edm.sendwalk.com/api/health | 检查 API 状态 |

## 🔐 安全建议

### 1. 强制 HTTPS

配置 Nginx 将所有 HTTP 请求重定向到 HTTPS（Certbot 会自动配置）。

### 2. HSTS 头

在 Nginx SSL 配置中添加：

```nginx
add_header Strict-Transport-Security "max-age=31536000; includeSubDomains" always;
```

### 3. 安全头

```nginx
add_header X-Frame-Options "SAMEORIGIN" always;
add_header X-Content-Type-Options "nosniff" always;
add_header X-XSS-Protection "1; mode=block" always;
add_header Referrer-Policy "no-referrer-when-downgrade" always;
```

### 4. 隐藏版本信息

```nginx
# 在 nginx.conf 的 http 块中添加
server_tokens off;
```

## ✅ 配置检查清单

部署完成后，确认以下项目：

- [ ] DNS 记录已添加（edm.sendwalk.com 和 api.edm.sendwalk.com）
- [ ] DNS 已生效（可以 ping 通）
- [ ] 后端 .env 配置正确（APP_URL、FRONTEND_URL 等）
- [ ] 前端 .env 配置正确（VITE_API_URL）
- [ ] 前端已重新构建（npm run build）
- [ ] Nginx 配置已更新并重启
- [ ] SSL 证书已申请并配置
- [ ] HTTP 访问可以正常重定向到 HTTPS
- [ ] 前端页面可以正常访问
- [ ] API 可以正常访问（测试 /api/health）
- [ ] 前端可以正常调用 API（无 CORS 错误）
- [ ] 可以正常登录和使用功能

## 📝 快速配置命令

```bash
# 1. 配置后端
cd /data/www/sendwalk/backend
cp ../config.env.production.example .env
sed -i 's|APP_URL=.*|APP_URL=https://edm.sendwalk.com|' .env
sed -i 's|FRONTEND_URL=.*|FRONTEND_URL=https://edm.sendwalk.com|' .env
sed -i 's|SANCTUM_STATEFUL_DOMAINS=.*|SANCTUM_STATEFUL_DOMAINS=edm.sendwalk.com|' .env
sed -i 's|SESSION_DOMAIN=.*|SESSION_DOMAIN=.edm.sendwalk.com|' .env

# 2. 配置前端
cd /data/www/sendwalk/frontend
echo "VITE_API_URL=https://api.edm.sendwalk.com" > .env
echo "VITE_APP_NAME=SendWalk" >> .env

# 3. 重新构建前端
npm run build

# 4. 配置 Nginx
sudo cp /data/www/sendwalk/nginx/api.conf /etc/nginx/conf.d/sendwalk-api.conf
sudo cp /data/www/sendwalk/nginx/frontend.conf /etc/nginx/conf.d/sendwalk-frontend.conf
sudo nginx -t && sudo systemctl restart nginx

# 5. 配置 SSL
sudo certbot --nginx -d edm.sendwalk.com
sudo certbot --nginx -d api.edm.sendwalk.com

# 6. 清除缓存
cd /data/www/sendwalk/backend
php artisan config:clear
php artisan config:cache

# 7. 测试
curl https://edm.sendwalk.com
curl https://api.edm.sendwalk.com/api/health
```

---

**域名配置完成！** ✅

现在您的 SendWalk 应用将通过以下地址访问：
- 前端：https://edm.sendwalk.com
- API：https://api.edm.sendwalk.com

