# 部署路径和域名配置

## 📂 项目部署路径

本项目将部署到：**`/data/www/sendwalk`**

## 🌐 使用的域名

- **前端**: `edm.sendwalk.com`
- **API**: `api.sendwalk.com`

所有配置文件和脚本已经更新为使用此路径和域名。

## 📋 已更新的文件

### **Nginx 配置**

✅ `nginx/api.conf`
```nginx
root /data/www/sendwalk/backend/public;
```

✅ `nginx/frontend.conf`
```nginx
root /data/www/sendwalk/frontend/dist;
```

### **Supervisor 配置**

✅ `supervisor/scheduler.conf`
```ini
command=/usr/bin/php /data/www/sendwalk/backend/artisan schedule:work
stdout_logfile=/data/www/sendwalk/backend/storage/logs/scheduler.log
```

✅ `supervisor/worker.conf`
```ini
command=/usr/bin/php /data/www/sendwalk/backend/artisan queue:manage-workers ...
stdout_logfile=/data/www/sendwalk/backend/storage/logs/manager.log
```

### **自动化脚本**

✅ `deploy.sh`
```bash
PROJECT_DIR="/data/www/sendwalk"
```

✅ `check-deployment.sh`
```bash
PROJECT_DIR="/data/www/sendwalk"
```

## 🚀 部署步骤

### 0. 配置 DNS（在域名服务商操作）

在域名服务商（如阿里云、腾讯云）添加以下 A 记录：

```
类型   主机记录    记录值
A      edm        服务器IP
A      api        服务器IP
```

验证 DNS 生效：

```bash
ping edm.sendwalk.com
ping api.sendwalk.com
```

> 💡 **提示**: DNS 生效通常需要 5-10 分钟，最长可能需要 24 小时

### 1. 创建项目目录

```bash
# 创建目录
sudo mkdir -p /data/www

# 设置权限
sudo chown -R www-data:www-data /data/www
```

### 2. 克隆代码

```bash
# 切换到目标目录
cd /data/www

# 克隆代码
git clone https://github.com/your-username/sendwalk.git

# 设置权限
sudo chown -R www-data:www-data /data/www/sendwalk
```

### 3. 配置后端

```bash
cd /data/www/sendwalk/backend

# 安装依赖
composer install --optimize-autoloader --no-dev

# 配置环境
cp .env.example .env
nano .env  # 编辑配置

# 生成密钥和迁移
php artisan key:generate
php artisan migrate --force

# 优化
php artisan config:cache
php artisan route:cache
php artisan view:cache

# 设置权限
sudo chown -R www-data:www-data storage bootstrap/cache
sudo chmod -R 775 storage bootstrap/cache
```

### 4. 配置前端

```bash
cd /data/www/sendwalk/frontend

# 安装依赖
npm install

# 配置环境变量
cat > .env << 'EOF'
VITE_API_URL=https://api.sendwalk.com
VITE_APP_NAME=SendWalk
EOF

# 构建
npm run build
```

### 5. 配置 Nginx

```bash
# 复制配置文件到 conf.d 目录
sudo cp /data/www/sendwalk/nginx/api.conf /etc/nginx/conf.d/sendwalk-api.conf
sudo cp /data/www/sendwalk/nginx/frontend.conf /etc/nginx/conf.d/sendwalk-frontend.conf

# 测试配置
sudo nginx -t

# 重启 Nginx
sudo systemctl restart nginx
```

### 6. 配置 Supervisor

```bash
# 复制配置文件
sudo cp /data/www/sendwalk/supervisor/scheduler.conf /etc/supervisor/conf.d/sendwalk-scheduler.conf
sudo cp /data/www/sendwalk/supervisor/worker.conf /etc/supervisor/conf.d/sendwalk-worker-manager.conf

# 重新加载配置
sudo supervisorctl reread
sudo supervisorctl update

# 启动所有进程
sudo supervisorctl start all

# 检查状态
sudo supervisorctl status
```

### 7. 配置 SSL

#### 使用已有的 Cloudflare 证书

```bash
# 证书文件已准备在：
# /data/www/ca/sendwalk.pem   (证书)
# /data/www/ca/sendwalk.key   (私钥)

# 确认证书文件存在
ls -lh /data/www/ca/sendwalk.pem
ls -lh /data/www/ca/sendwalk.key

# 设置正确的权限
sudo chown root:root /data/www/ca/sendwalk.pem
sudo chown root:root /data/www/ca/sendwalk.key
sudo chmod 644 /data/www/ca/sendwalk.pem
sudo chmod 600 /data/www/ca/sendwalk.key

# Nginx 配置已包含 SSL 设置，直接重启即可
sudo nginx -t
sudo systemctl restart nginx
```

> 💡 详细的 SSL 配置说明请查看：[SSL证书配置说明.md](./SSL证书配置说明.md)

### 8. 验证部署

```bash
cd /data/www/sendwalk
./check-deployment.sh
```

## 📁 目录结构

```
/data/www/sendwalk/
├── backend/
│   ├── app/
│   ├── config/
│   ├── public/          # Nginx root for API
│   ├── storage/
│   │   └── logs/
│   │       ├── laravel-YYYY-MM-DD.log
│   │       ├── scheduler.log
│   │       └── manager.log
│   ├── .env
│   └── artisan
├── frontend/
│   ├── dist/            # Nginx root for frontend
│   │   ├── index.html
│   │   ├── assets/
│   │   └── favicon.svg
│   ├── src/
│   └── .env
├── nginx/
│   ├── api.conf
│   └── frontend.conf
├── supervisor/
│   ├── scheduler.conf
│   └── worker.conf
├── deploy.sh
├── check-deployment.sh
└── README-DEPLOYMENT.md
```

## 🔍 重要路径

### **应用路径**

| 项目 | 路径 |
|------|------|
| 项目根目录 | `/data/www/sendwalk` |
| 后端代码 | `/data/www/sendwalk/backend` |
| 后端入口 | `/data/www/sendwalk/backend/public` |
| 前端代码 | `/data/www/sendwalk/frontend` |
| 前端构建 | `/data/www/sendwalk/frontend/dist` |

### **日志路径**

| 日志 | 路径 |
|------|------|
| Laravel | `/data/www/sendwalk/backend/storage/logs/laravel-*.log` |
| Scheduler | `/data/www/sendwalk/backend/storage/logs/scheduler.log` |
| Worker | `/data/www/sendwalk/backend/storage/logs/manager.log` |
| Nginx API | `/var/log/nginx/sendwalk-api-*.log` |
| Nginx Frontend | `/var/log/nginx/sendwalk-frontend-*.log` |

### **配置路径**

| 配置 | 路径 |
|------|------|
| 后端环境变量 | `/data/www/sendwalk/backend/.env` |
| 前端环境变量 | `/data/www/sendwalk/frontend/.env` |
| Nginx API | `/etc/nginx/conf.d/sendwalk-api.conf` |
| Nginx Frontend | `/etc/nginx/conf.d/sendwalk-frontend.conf` |
| Supervisor Scheduler | `/etc/supervisor/conf.d/sendwalk-scheduler.conf` |
| Supervisor Worker | `/etc/supervisor/conf.d/sendwalk-worker-manager.conf` |

## 🛠️ 常用命令

### **进入项目目录**

```bash
cd /data/www/sendwalk
```

### **更新代码**

```bash
cd /data/www/sendwalk
./deploy.sh production
```

### **检查状态**

```bash
cd /data/www/sendwalk
./check-deployment.sh
```

### **查看日志**

```bash
# Laravel 日志
tail -f /data/www/sendwalk/backend/storage/logs/laravel-$(date +%Y-%m-%d).log

# Scheduler 日志
tail -f /data/www/sendwalk/backend/storage/logs/scheduler.log

# Worker 日志
tail -f /data/www/sendwalk/backend/storage/logs/manager.log
```

### **重启服务**

```bash
# 重启 PHP-FPM
sudo systemctl restart php8.3-fpm

# 重启 Nginx
sudo systemctl restart nginx

# 重启 Supervisor 进程
sudo supervisorctl restart all
```

## ⚠️ 注意事项

### **权限设置**

确保以下目录权限正确：

```bash
# 设置所有者
sudo chown -R www-data:www-data /data/www/sendwalk

# 设置存储目录权限
sudo chmod -R 775 /data/www/sendwalk/backend/storage
sudo chmod -R 775 /data/www/sendwalk/backend/bootstrap/cache
```

### **SELinux（如果启用）**

如果服务器启用了 SELinux，需要设置正确的上下文：

```bash
# 设置 SELinux 上下文
sudo semanage fcontext -a -t httpd_sys_rw_content_t "/data/www/sendwalk/backend/storage(/.*)?"
sudo restorecon -Rv /data/www/sendwalk/backend/storage
```

### **防火墙**

确保防火墙允许 HTTP/HTTPS 访问：

```bash
sudo ufw allow 80/tcp
sudo ufw allow 443/tcp
```

## ✅ 快速检查清单

部署完成后，确认以下项目：

- [ ] 项目代码在 `/data/www/sendwalk`
- [ ] 后端依赖已安装
- [ ] 数据库已迁移
- [ ] 前端已构建
- [ ] Nginx 配置正确（指向 `/data/www/sendwalk`）
- [ ] Supervisor 配置正确（指向 `/data/www/sendwalk`）
- [ ] 所有服务正常运行
- [ ] 文件权限正确（www-data:www-data）
- [ ] SSL 证书配置（如需要）
- [ ] 可以访问前端和 API
- [ ] Worker 正常处理任务
- [ ] 日志正常记录

## 🔗 相关文档

- **域名配置详细指南**: [域名配置说明.md](./域名配置说明.md) - **推荐首先阅读**
- **快速部署命令**: [快速部署命令.sh](./快速部署命令.sh) - 所有命令速查
- **完整部署指南**: [生产环境部署指南.md](./生产环境部署指南.md)
- **文档索引**: [README-DEPLOYMENT.md](./README-DEPLOYMENT.md)

---

**所有配置文件和脚本已更新为使用 `/data/www/sendwalk` 路径和 `edm.sendwalk.com` 域名，可以直接使用！** ✅

